...
However, that scheme can be evaded by the adversary with manipulated malware grayscale image or binary code.
...
The valuable regions still remain which contribute to classification in both manipulated targets, grayscale image and binary code, after the attacks.
...
This is because adversaries have to scan IoT devices and propagate their malware before attacks itself.
As a result, these phases cause abnormal traffic which cannot be seen in benign cases.
Based on the fact, those schemes utilize the features such as destination IP address during a certain time, port number and so on, at a gateway of the network.
The features are extracted from incoming packets in the scanning/propagation phase.
Although router-side schemes are useful, these schemes are subject to internal attacks of the local network itself because the packets do not go through the router.
...
Most of the device-side schemes utilze various features which can be extracted statically because IoT devices are not suitable for dynamic detections due to their limited computing and storage capabilities.
...
Su et al. utilezes GrayScale Image (GSI) converted from raw bytes of each malware binary code for its CNN \cite{previous}.
...
As a result, Adversarial Malware (AM) and Obfuscated Malware (OM) are created by applying these techniques to GSI or Binary Code (BC) of malware, respectively.
...
Kumar et al. \cite{net} propose the scheme focusing on network traffic while the scanning and propagation phase.
...
That scheme can detect and isolate the bots before attacks because the scanning and propagation phase of the botnet life-cycle stretches over many months, which results in abnormal traffic that cannot be seen in benign cases.
...
Device-side detection schemes are conducted by utilizing the features extracted from malware file itself at each device.
...
Hwang et al. also use string information from binary code \cite{hwang}.
...
\myfigurename~\ref{fig:cfg} shows the overview of device-side detection using CFG.
...
In the previous scheme \cite{previous}, a malware binary code is reformatted as an 8-bit sequence and then is converted to a GSI which has one channel and pixel values from 0 to 255.
...
Although the previous scheme can cope with spreading IoT malware with device-side detection in more practical means, it is susceptible to evasive techniques against the inputs of the CNN, GSIs and binary code \cite{attack1, attack2}.
Adversaries can evade the CNN based detection with two evasive techniques against each input as shown in \myfigurename~\ref{fig:ov}.
Malwares whose GSIs are manipulated to evade the detection are called Adversarial Malware (AM) and also malware whose binary code is manipulated are called Obfuscated Malware (OM) in this research.  
...
The GSIs fed into the network as inputs in the previous scheme have pixels that match the BC, which is inserted into the source code by the compiler and has nothing to do with runtime behaviour \cite{am}.
...
The two GSIs shown in \myfigurename~\ref{fig:amGSI} are the actual GSIs of a malware before and after adding noise.
...
\begin{figure}[t]
 ...
 \label{fig:amGSI}
\end{figure}
\subsubsection{Vulnerability against binary code manipulation}
The binary code which converts to a GSI can also be manipulated to evade the CNN detection by being obfuscated \cite{om}.
Due to the small computational resources, obfuscation techniques with packing tools, which also can compress the BC while encrypting, are tend to be used in IoT malware obfuscation field.
...
In addition to the fact that malware obfuscated with the packing tools can be recovered and analyzed only at runtime, the encryption method differs depending on the packing tool.Thus, binary code manipulation attack is a threat especially for IoT devices which are not suitable for dynamic analysis due to the limited computing resources since there is no way to statically analyze them in advance.
...
In order to identify the JPs, we focus on the fact that binary code regions that are necessary or unnecessary at runtime can be classified according to informataion of each section.
...
In particular, a binary code can be reformatted to a sequence whose elements are 8-bit strings.
